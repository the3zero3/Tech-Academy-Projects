...
class ParentWindow(Frame):
    def __init__(self, master):
        ...
        # padx and pady are the same as the button to ensure they will line up
    ...
    def transferFiles(self):
        # Sets a variable with the current timestamp in seconds from epoch
        ...
        for i in source_files:
            path = source + '/' + i
            # Sets a variable with the file timestamp in seconds from epoch
            mod_time = os.path.getmtime(path)
            # Checks if time since modification is bigger than 86400 seconds or 24 hours
            if (now_timestamp - mod_time) < 86400:
                # Moves each file from the source to the destination
                shutil.move(source + "/" + i, destination)
                print(i + " was successfully transferred.")
            else:
                print(i + " was not modified in the last 24 hours.")
